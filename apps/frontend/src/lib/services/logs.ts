...
interface CachedProposals {
	lastBlock: string;
	events: CachedProposalEvent[];
}

const PROPOSAL_CACHE_KEY = 'daa:proposals';

export function getCachedProposals(): CachedProposals | null {
	try {
...
export function setCachedProposals(data: CachedProposals) {
	try {
		localStorage.setItem(PROPOSAL_CACHE_KEY, JSON.stringify(data));
	} catch {
		// storage full or unavailable
	}
}
